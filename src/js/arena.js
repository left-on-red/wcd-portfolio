(function() {
    generatorScripts.arena = async function() {
        let xml = await getFile('../data/arena.xml');
        ...
        let html = document.getElementById('arenaContainer');
        let table = document.createElement('table');
        table.classList.add('arenaTable');
        ...
        if (!localStorage.getItem(`2-name`)) { localStorage.setItem(`2-name`, '') }
        nameBox.value = localStorage.getItem(`2-name`);
        nameBox.addEventListener('input', function(event) { localStorage.setItem(`2-name`, event.target.value) });
        ...
        if (!localStorage.getItem(`2-id`)) { localStorage.setItem(`2-id`, '') }
        idBox.value = localStorage.getItem(`2-id`);
        idBox.addEventListener('input', function(event) { localStorage.setItem(`2-id`, event.target.value) });
        ...
        function generateRow(entry, number) {
            ...
            special = attributes.includes('specialCall') == true ? 'Call' : special;
            ...
            for (let i = tds.length - 3; i < tds.length; i++) {
                let id = tds.length == 7 ? `2-${count}-${i-3}` : `2-${count}-${i-2}`;
    ...
    exportScripts.arena = function() {
        let table = document.getElementsByClassName('arenaTable')[0];
        ...
        post('/pdf/arena', final).then(function(response) {
            if (response == 'success') { ref.location = '/pdf/arena' }
            else {ref.close() }
        });
    }
})();